...
5. **Pegue a String de Conexão (DATABASE_URL)**:
   ...
   - **MUITO IMPORTANTE:** Se você esqueceu ou precisa mudar a senha do banco, vá no menu lateral esquerdo em **Project Settings** (Engrenagem) e depois clique em **Database**. Role para baixo até achar a seção "Database Password" e clique em **Reset Password**.
   - **MUITO IMPORTANTE 2:** Ao criar a senha do banco, use apenas **letras e números**. Caracteres especiais como `+`, `@`, `/` ou `#` na senha quebram a URL de conexão se não forem codificados.
   - **MUITO IMPORTANTE 3:** Ao colar essa URL no Render, adicione `?pgbouncer=true&connection_limit=1` no final dela. Sem isso, o Prisma pode falhar na hora de criar as tabelas (migrate) no Render.
